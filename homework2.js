...
function changeColor(color) {
    if (color != null) {
        // validation
        if (color.charAt(0) != "#") {
            console.log("I'm sorry, that is not a valid hex number. Your first character was "
                + color.charAt(0) + ", and not a #. Please try again.");
            return;
        }
        for (i = 1; i <= 6; i++) {
            if (validChars.indexOf(color.charAt(i)) == -1) {
                console.log("I'm sorry, that is not a valid hex number. Your first invalid character was "
                    + color.charAt(i) + ". Please try again.");
                break;
            }
        }
